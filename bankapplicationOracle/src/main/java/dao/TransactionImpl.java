			...
			while(resultSet.next()) {
				if(trans.getToAccountNumber()==resultSet.getInt(1)) {
					...
					if(balance > trans.getAmountTransfer()) {
						...
						PreparedStatement preparedStatement4=database.database().prepareStatement("insert into transaction_details(from_account_number,to_account_number,amount_transfer) values(?,?,?)");
